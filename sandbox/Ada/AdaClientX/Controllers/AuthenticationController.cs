...
public class AuthenticationController : PlantController
{
    ...
    public const string SsoLoginActionName = "SsoLogin";
    public const string SsoLoginRouteName = "SsoLogin";
    public const string SsoLoginRouteTemplate = "/sso/login";

    public const string SsoLoginCallbackActionName = "SsoLoginCallback";
    public const string SsoLoginCallbackRouteName = "SsoLoginCallback";
    public const string SsoLoginCallbackRouteTemplate = "/sso/callback/login";

    public const string SsoLoginLocalProvider = "local";

    public const string SsoLogoutActionName = "SsoLogout";
    public const string SsoLogoutRouteName = "SsoLogout";
    public const string SsoLogoutRouteTemplate = "/sso/logout";

    public const string SsoLogoutCallbackActionName = "SsoLogoutCallback";
    public const string SsoLogoutCallbackRouteName = "SsoLogoutCallback";
    public const string SsoLogoutCallbackRouteTemplate = "/sso/callback/logout";

    [ActionName(SsoLoginActionName)]
    [HttpGet]
    [Route("~" + SsoLoginRouteTemplate + "/{provider}", Name = SsoLoginRouteName)]
    public IActionResult SsoLogin([FromRoute] string? provider, [FromQuery] string? redirectUri)
    {
        AuthenticationProperties properties = new()
        {
            RedirectUri = Url.IsLocalUrl(redirectUri) ? redirectUri : "/"
        };

        return Challenge(properties, OpenIddictClientAspNetCoreDefaults.AuthenticationScheme);
    }

    [ActionName(SsoLoginCallbackActionName)]
    [HttpGet, HttpPost]
    [Route("~" + SsoLoginCallbackRouteTemplate + "/{provider}", Name = SsoLoginCallbackRouteName)]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> SsoLoginCallback([FromRoute] string? provider)
    {
    ...
    [ActionName(SsoLogoutActionName)]
    [HttpPost]
    [Route("~" + SsoLogoutRouteTemplate + "/{provider}", Name = SsoLogoutRouteName)]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> SsoLogout([FromRoute] string? provider, [FromQuery] string? redirectUri)
    {
        ...
        if (result is not { Succeeded: true })
        {
            ...
            return Redirect(Url.IsLocalUrl(redirectUri) ? redirectUri : "/");
        }
    ...
    [ActionName(SsoLogoutCallbackActionName)]
    [HttpGet, HttpPost]
    [Route("~" + SsoLogoutCallbackRouteTemplate + "/{provider}", Name = SsoLogoutCallbackRouteName)]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> SsoLogoutCallback([FromRoute] string? provider)
    {
        ...
        if (result.Properties!.RedirectUri is string redirectUri && Url.IsLocalUrl(redirectUri))
        {
